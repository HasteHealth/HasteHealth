...
Haste Health can validate resources against **FHIR profiles** — `StructureDefinition` resources that constrain a base FHIR type with tighter cardinality, fixed values, patterns, and slicing rules. This is how implementation guides like **US Core** define what a conformant `Patient` or `Observation` must look like, and Haste Health enforces those rules automatically at write time.
...
4. Rejects the write with a `400` and an `OperationOutcome` if any element fails validation.
...
1. Download the profiles you need from the [US Core StructureDefinitions](https://hl7.org/fhir/us/core/profiles.html) page (or the full package from the [FHIR package registry](https://hl7.org/fhir/us/core/)).
...
- **[Create Resource →](/docs/api/rest_api/fhir/endpoints/create)**
