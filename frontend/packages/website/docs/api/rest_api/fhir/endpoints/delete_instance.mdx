---
id: delete-instance
title: delete-instance
description: How to delete a specific FHIR resource instance from the Haste Health server using the CLI, TypeScript client, or cURL.
sidebar_position: 5
---

# Delete Resource Instance

The FHIR [delete operation](https://www.hl7.org/fhir/http.html#delete) allows you to delete a specific resource instance from the FHIR server. This is done using an HTTP DELETE request to the endpoint corresponding to the resource type and ID.

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<Tabs>
<TabItem value="cli" label="CLI" default>

Delete a Patient resource using the Haste Health CLI:

```bash
haste-health api delete-instance Patient 12345
```

The CLI will prompt for confirmation before deleting the resource.

</TabItem>
<TabItem value="typescript" label="TypeScript">

Delete a Patient resource using the TypeScript client:

```typescript
import { AsynchronousClient } from '@haste-health/client';
import { R4 } from '@haste-health/fhir-types/versions';

// Assuming you have a configured client
const client: AsynchronousClient<{}> = /* ... */;

await client.delete_instance(
  {},
  R4,
  'Patient',
  '12345'
);

console.log('Patient deleted successfully');
```

</TabItem>
<TabItem value="curl" label="cURL">

Delete a Patient resource using cURL:

```bash
curl -X DELETE "https://api.haste.health/w/[tenant]/[project]/api/v1/fhir/Patient/12345" \
     -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```

</TabItem>
</Tabs>
Replace `[tenant]` with your tenant name and `[project]` with your project ID.
<div className="border border-brand-100" />



