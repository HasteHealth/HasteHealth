---
title: SMART on FHIR
description: How SMART on FHIR builds on OAuth 2.0 to provide healthcare-specific authorization, patient/user context, and standardized scopes in Haste Health.
sidebar_position: 5
tags:
  - authentication
  - smart-on-fhir
...
:::caution EHR Launch is not yet supported in Haste Health
This feature is not yet supported in Haste Health. The following is for informational purposes only.
:::
...
https://app.example.com/launch?
  iss=https://api.haste.health/w/{tenant}/{project}/api/v1/fhir
...
GET /w/{tenant}/{project}/api/v1/oidc/auth/authorize?
  ...
  state=random-state&
  aud=https://api.haste.health/w/{tenant}/{project}/api/v1/fhir&
  launch=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```
