...
description: How to use OAuth 2.0 refresh tokens in Haste Health to obtain new access tokens without requiring the user to re-authenticate.
sidebar_position: 3
tags:
  - authentication
  - oidc
...
sequenceDiagram
    ...
    User->>AuthServer: GET /w/{tenant}/{project}/api/v1/oidc/auth/authorize
...
```http
POST /w/{tenant}/{project}/api/v1/oidc/auth/token
